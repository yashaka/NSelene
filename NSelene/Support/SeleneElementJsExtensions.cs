namespace NSelene.Support.SeleneElementJsExtensions
{
    public static class SeleneElementJsExtensions
    {
        ...
        {
            element.Wait.For(new _Lambda<SeleneElement, object>(
                $"JsClick(centerXOffset: {centerXOffset}, centerYOffset: {centerYOffset})",
                self =>
                {
        ...
        {
            // TODO: probably we have to wait here for something... for what?:)
            element.ExecuteScript(
        ...
        public static SeleneElement JsSetValue(this SeleneElement element, string value)
        {
            ...
            if (element.Config.WaitForNoOverlapFoundByJs ?? false)
            {
                element.Wait.For(new _Lambda<SeleneElement, object>(
                    $"JsSetValue({value})",
                    self =>
